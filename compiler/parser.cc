...
bool Parser::EmitValue(const Token& value) {
  switch (value.kind) {
...
void Parser::EmitOp(const Token& op) {
  switch (op.kind) {
    case TokenKind::kAdd:
...
bool Parser::ParseExpression(Token& token) {
  return ParseAssignment(token);
}

bool Parser::ParseAssignment(Token& token) {
...
bool Parser::ParseComparison(Token& token) {
...
bool Parser::ParseAdditive(Token& token) {
...
bool Parser::ParseMultiplicative(Token& token) {
  if (!ParseValue(token))
    return false;
...
bool Parser::ParseValue(Token& token) {
  switch (token.kind) {
    case TokenKind::kNumber:
    case TokenKind::kString:
    case TokenKind::kIdent:
...
bool Parser::ParseCall(Token& token, Token fn_name) {
  token = tokenizer_.next();  // consume '('
